		...
		<section class="section-background">
			...
			<section class="section-top-menus">
				<div class="product-info">
					작성자 :
					<c:out value="${sellPost.memberId}" />
				</div>
				<form action="delete.tr" method="get">
					<c:if test="${sellPost.memberNumber == sessionScope.memberNumber}">
						<button class="post-button-delete" type="button" onclick="">삭제하기</button>
					</c:if>
				</form>
			</section>
			...
			<section class="section-middle-menus">
				<div class="product-info-container">
					...
					<div class="product-image">
						<c:choose>
							<c:when test="${not empty sellPost.attachment}">
								<c:forEach var="file" items="${sellPost.attachment}"
									varStatus="status">
					...
					<!-- 카테고리 -->
					<div class="product-info">
						<c:out value="${sellPost.teamName}" />
						>
						<c:out value="${sellPost.categoryName}" />
					</div>
					...
					<div class="product-info">
						...
						<fmt:formatDate value="${sellPost.sellPostCreationDate}"
							pattern="yyyy.MM.dd" />
						<br> 수정일 :
						<fmt:formatDate value="${sellPost.sellPostUpdate}"
							pattern="yyyy.MM.dd" />
					</div>
				</div>
				...
				<div class="product-detail-container">
					...
					<div class="product-description">
						상품 설명 :
						<div>
							<c:out value="${sellPost.sellPostContent}" escapeXml="false" />
						</div>
					</div>
					...
					<div class="trading-info-container">
						<div class="trading-info">
							거래 지역 :
							<div>
								<c:out value="${sellPost.tradingArea}" />
							</div>
						</div>
						<div class="trading-info">
							거래 방식 :
							<div>
								<c:out value="${sellPost.dealType}" />
							</div>
						</div>
					</div>
					<div class="line"></div>

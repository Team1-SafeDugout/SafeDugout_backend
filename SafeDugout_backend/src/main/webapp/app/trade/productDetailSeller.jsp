<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html lang="en">

<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>상품 디테일 페이지(판매자)</title>
<link rel="stylesheet" href="${pageContext.request.contextPath}/assets/css/trade/productDetailSeller.css">
<link rel="stylesheet" href="${pageContext.request.contextPath}/assets/css/header.css">
<link rel="stylesheet" href="${pageContext.request.contextPath}/assets/css/footer.css">
<script defer src="${pageContext.request.contextPath}/assets/js/trade/productDetailSeller.js"></script>
</head>

<!-- 바디 -->

<body>
	<!-- 헤더 -->
	<jsp:include page="/header.jsp" />
	<!-- 폼 태그 -->
	<form action="" method="get">
		<!-- 검색 컨테이너 -->
		<div class="search-container">
			<!-- 입력 텍스트 -->
			<input type="text" placeholder="검색어를 입력해주세요">
			<!-- 버튼 -->
			<button type="button"
				onclick="location.href='./tradeSearchResult.html'">검색</button>
		</div>
	</form>
	<!-- 메인 -->
	<main>
		<!-- 배경 -->
		<section class="section-background">
			<!-- 상단 요소들 -->
			<section class="section-top-menus">
				<!-- 상품 정보 출력값 -->
				<div class="product-info">작성자 : jsj3653</div>
				<!-- 폼 태그 -->
				<form action="" method="get">
					<!-- 수정하기 버튼 -->
					<a href="./productRegister.html"><button
							class="post-button-update" type="button">수정하기</button></a>
					<!-- 삭제하기 버튼 -->
					<a><button class="post-button-delete" type="button">
							삭제하기</button></a>
				</form>
			</section>

			<!-- 가운데 메뉴 -->
			<section class="section-middle-menus">
				<!-- 상품 설명 컨테이너 -->
				<div class="product-info-container">
					<!-- 상품 이미지 -->
					<div class="product-image">
						<!-- 상품 이미지 img 태그 -->
						<img
							src="${pageContext.request.contextPath}/assets/img/tradeImg/product12.png"
							alt="">
						<!-- 화살표 버튼 -->
						<div class="product-slide-buttons">
							<!-- 왼쪽 화살표 버튼 -->
							<div id="left-slide-button">◁</div>
							<!-- 오른쪽 화살표 버튼 -->
							<div id="right-slide-button">▷</div>
						</div>
					</div>
					<!-- 카테고리(팀, 종류) -->
					<div class="product-info">두산>모자</div>
					<!-- 등록 날짜 -->
					<div class="product-info">
						게시일 : <span>2025.08.12</span><br> 수정일 : <span>2025.08.14</span>
					</div>
				</div>

				<!-- 상품 디테일 컨테이너 -->
				<div class="product-detail-container">
					<!-- 상품 이름 박스 -->
					<div class="product-name">
						상품 이름 :
						<!-- 상품 이름 -->
						<div>[두산베어스] 오리지널 모자</div>
					</div>
					<!-- 상품 설명 박스 -->
					<div class="product-description">
						상품 설명 :
						<!-- 상품 설명 -->
						<div>
							두산 베어스 오리지널 모자입니다<br>
							<br> 벨크로 형식으로 사이즈 조절이 가능합니다 <br> 정상 판매가 30000, 중고거래
							15000에 판매합니다
						</div>
					</div>

					<!-- 거래 정보 컨테이너 -->
					<div class="trading-info-container">
						<!-- 거래 정보 -->
						<div class="trading-info">
							거래 지역 :
							<!-- 거래 지역 -->
							<div>구리</div>
						</div>

						<!-- 거래 정보 -->
						<div class="trading-info">
							거래 방식 :
							<!-- 거래 지역 -->
							<div>직거래</div>
						</div>
					</div>

					<!-- 검은선 -->
					<div class="line"></div>

					<!-- 상품 가격 -->
					<div class="product-price">
						상품 가격 :
						<!-- 포인트 값 -->
						<div>15000 포인트</div>
					</div>
				</div>
			</section>

			<!-- 하단 컨테이너 -->
			<section class="section-bottom-menus">
				<div>
					<!-- 폼 태그 -->
					<form action="" method="get">
						<!-- 목록으로 버튼 -->
						<button type="button" class="post-button"
							onclick="location.href = './tradeMain.html' ">목록으로</button>
					</form>
				</div>
			</section>
		</section>
	</main>
	<!-- 푸터 -->
	<jsp:include page="${pageContext.request.contextPath}/footer.jsp" />
	<!-- 스크립트 -->
	<script src="${pageContext.request.contextPath}/assets/js/include.js"></script>
</body>

</html>
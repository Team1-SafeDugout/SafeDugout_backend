...
<body>
  <jsp:include page="/app/communityHtml/headerHtml/teamHeader.jsp" />
  <main>
    <!-- 게시글 목록 -->
...
<%--           <!-- 검색 폼 -->
      ...
      <div class="pagination">
        <ul>
          <c:if test="${prev}">
            <li><a href="${pageContext.request.contextPath}/community/teamCommunityFrontController.tc?category=board&action=postlist&page=${startPage - 1}">&lt;</a></li>
          </c:if>
          ...
          <c:forEach var="i" begin="${realStartPage}" end="${endPage}">
            <c:choose>
              <c:when test="${i != page}">
                <li><a href="${pageContext.request.contextPath}/community/teamCommunityFrontController.tc?category=board&action=postlist&page=${i}">
                  <c:out value="${i}" />
                </a></li>
              </c:when>
              <c:otherwise>
          ...
          <c:if test="${next}">
            <li><a href="${pageContext.request.contextPath}/community/teamCommunityFrontController.tc?category=board&action=postlist&page=${endPage + 1}">&gt;</a></li>
          </c:if>
        </ul>
      </div>
    </div>
  ...
  <jsp:include page="/app/communityHtml/teamFooter/teamFooter.jsp" />

  <script>
    let memberNumber = ${sessionScope.memberNumber != null ? sessionScope.memberNumber : 0};
    let teamNumber = ${sessionScope.teamNumber != null ? sessionScope.teamNumber : 0};
  </script>
</body>
</html>

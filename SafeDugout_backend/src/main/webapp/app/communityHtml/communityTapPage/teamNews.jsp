...
<title>불펜토크</title>
<link rel="stylesheet"
	href="${pageContext.request.contextPath}/assets/css/communityCss/communityTapPage/teamNews.css">
<c:choose>
		...
		<div class="team-news-container">
				    ...
				    <c:when test="${not empty postList}">
				        <c:forEach var="community" items="${postList}">
				            <li class="team-news-list">
				                <a href="${pageContext.request.contextPath}/community/teamCommunityFrontController.tc?category=news&action=newsdetail&postNumber=${community.postNumber}"> 
				                    <c:choose>
				                        <c:when test="${not empty community.attachment}">
				                            <img src="${pageContext.request.contextPath}/upload/${community.attachment[0].imgPath}" 
				                                 alt="${community.postTitle}" title="${community.postTitle}">
				                        </c:when>
				                        <c:otherwise>
				                            <img src="${pageContext.request.contextPath}/assets/img/communityImg/${community.teamName}.png" 
				                                 alt="${community.postTitle}" title="${community.postTitle}">
				                        </c:otherwise>
				                    </c:choose>
			...
			<div class="pagination">
				<ul>
					<c:if test="${prev}">
						<li><a
							href="${pageContext.request.contextPath}/community/teamCommunityFrontController.tc?category=news&action=news&page=${startPage - 1}"
							class="prev">&lt;</a></li>
					</c:if>
					...
					<c:forEach var="i" begin="${realStartPage}" end="${endPage}">
					    <c:choose>
					        <c:when test="${i != page}">
					            <li>
					                <a href="${pageContext.request.contextPath}/community/teamCommunityFrontController.tc?category=news&action=news&page=${i}">
					                    <c:out value="${i}" />
					                </a>
					            </li>
					        </c:when>
					...
					<c:if test="${next}">
						<li><a
							href="${pageContext.request.contextPath}/community/teamCommunityFrontController.tc?category=news&action=news&page=${endPage + 1}"
							class="next">&gt;</a></li>
					</c:if>
				</ul>
			</div>

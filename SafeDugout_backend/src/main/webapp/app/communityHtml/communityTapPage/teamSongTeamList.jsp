...
<title>불펜토크 - 팀 응원가</title>
<link rel="stylesheet"
	href="${pageContext.request.contextPath}/assets/css/communityCss/communityTapPage/teamSongTeamList.css">
		...
		<div class="team-song-list-team">
			...
			<div class="pagination">
				<ul>
					<c:if test="${prev}">
						<li><a
							href="${pageContext.request.contextPath}/community/teamCommunityFrontController.tc?category=song&action=teamsong&page=${startPage - 1}">&lt;</a></li>
					</c:if>
					...
					<c:forEach var="i" begin="${realStartPage}" end="${endPage}">
						<c:choose>
							<c:when test="${i != page}">
								<li><a
									href="${pageContext.request.contextPath}/community/teamCommunityFrontController.tc?category=song&action=teamsong&page=${i}">
										<c:out value="${i}" />
								</a></li>
							</c:when>
							<c:otherwise>
					...
					<c:if test="${next}">
						<li><a
							href="${pageContext.request.contextPath}/community/teamCommunityFrontController.tc?category=song&action=teamsong&page=${endPage + 1}">&gt;</a></li>
					</c:if>
				</ul>
			</div>
		</div>

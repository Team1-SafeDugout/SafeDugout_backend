package com.bullPenTalk.app.main.dao;

import java.util.List;
import java.util.Map;

import org.apache.ibatis.session.SqlSession;

import com.bullPenTalk.app.dto.MainDTO;
import com.bullPenTalk.app.dto.TeamMainDTO;
import com.bullPenTalk.config.MyBatisConfig;

public class TeamMainDAO {
	public SqlSession sqlSession;

	public TeamMainDAO() {
		sqlSession = MyBatisConfig.getSqlSessionFactory().openSession(true);
	}

	//팀 뉴스 목록 조회
	public List<TeamMainDTO> selectNewsList() {
		return sqlSession.selectList("teamMain.selectNewsList");
	}
	
	//팀 게시글 목록 조회
	public List<TeamMainDTO> selectPostList() {
		return sqlSession.selectList("teamMain.selectPostList");
	}
	
	//팀 유튜브 썸네일 목록 조회
	public List<TeamMainDTO> selectYouTubeList() {
		return sqlSession.selectList("teamMain.selectYouTubeList");
	}
	
	//팀별 경기 일정 목록 조회
	public List<TeamMainDTO> selectTeamSchedule(int teamNumber) {
		return sqlSession.selectList("teamMain.selectTeamSchedule", teamNumber);
	}
	
	//팀 순위 목록 조회
	public List<TeamMainDTO> selectTeamRank() {
		return sqlSession.selectList("teamMain.selectTeamRank");
	}
}

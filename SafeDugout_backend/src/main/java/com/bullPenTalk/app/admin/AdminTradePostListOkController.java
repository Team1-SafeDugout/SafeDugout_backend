...
public class AdminTradePostListOkController implements Execute{

	@Override
	public Result execute(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		AdminTradeDAO adminTradeDAO = new AdminTradeDAO();
		...
		List<AdminTradeListDTO> boardList = adminTradeDAO.selectInfo(pageMap);
		request.setAttribute("boardList", boardList);
		
		// 페이징 정보 설정
		// BoardMapper.xml의 getTotal을 이용하여 전체 게시글 개수 조회
		// 실제 마지막 페이지 번호(realEndPage)를 계산함	
		int total = adminTradeDAO.getTotal();
		...
		result.setPath("/app/admin/adminMenu/adminManageTrading.jsp");

...
public class BuyController {
	...
	public Result payment(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		if (buyerPoint < pricePoint) {
			...
			result.setPath(
					request.getContextPath() + "/trade/sellPostFrontController2.tr?category=buy&action=charging");
		...
		if (success) {
			...
			// 상품 리스트 페이지로 이동
			result.setPath(
					request.getContextPath() + "/trade/sellPostFrontController2.tr?category=allproduct&action=list");
			result.setRedirect(true);
		} else {
			// 결제 실패 시 상품 디테일 페이지로 이동
			result.setPath(request.getContextPath()
					+ "/trade/sellPostFrontController2.tr?category=allproduct&action=detail&sellPostNumber="
					+ sellPostNumber);
			result.setRedirect(true);
		}
	...
	public Result chargingOk(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		// 입력값 확인
		if (memberNumberStr == null || chargePointStr == null || memberNumberStr.trim().isEmpty()
				|| chargePointStr.trim().isEmpty()) {
		...
		try {
			...
			org.jsoup.Connection.Response jsoupResponse = org.jsoup.Jsoup
					.connect("https://api.iamport.kr/users/getToken").ignoreContentType(true)
					.method(org.jsoup.Connection.Method.POST).data("imp_key", impKey).data("imp_secret", impSecret)
					.execute();
			...
			org.jsoup.Connection.Response paymentResponse = org.jsoup.Jsoup
					.connect("https://api.iamport.kr/payments/" + impUid).ignoreContentType(true)
					.header("Authorization", "Bearer " + accessToken).method(org.jsoup.Connection.Method.GET).execute();
			...
			if (update > 0) {
				...
				// 성공 페이지 이동
				result.setPath(request.getContextPath()
						+ "/trade/sellPostFrontController2.tr?category=allproduct&action=list");
				result.setRedirect(true);

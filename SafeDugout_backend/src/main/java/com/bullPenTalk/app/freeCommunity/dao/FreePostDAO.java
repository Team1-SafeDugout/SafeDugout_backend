...
public class FreePostDAO {
	 ...
	 // 모든 게시글 가져오기
		public List<FreePostDTO> select(Map<String, Integer> pageMap) {
			System.out.println("전체 커뮤니티 게시글 조회하기 - select 메소드 실행 : " + pageMap);
			List<FreePostDTO> list = sqlSession.selectList("FreeCommunityPostMapper.select", pageMap);
			System.out.println("조회결과 : " + list);
			return list;
		}
	    
		/*
		 * // 게시글 총 개수 가져오기 public int getTotal() {
		 * System.out.println("게시글 총 개수 조회 - getTotal 메소드 실행"); return
		 * sqlSession.selectOne("board.getTotal"); }
		 * 
		 * // 게시글 추가 후 자동으로 생성된 boardNumber 반환 -> 파일 테이블에서도 써야하기 때문에 public int
		 * insertBoard(FreePostDTO freePostDTO) { int insert =
		 * sqlSession.insert("board.insert", FreePostDTO);
		...
		// 게시글 상세 페이지 조회 메소드
		public FreePostDTO detailSelect(int PostNumber) {
			System.out.println("게시글 상세 페이지 조회(단건조회)");
			return sqlSession.selectOne("FreeCommunityPostMapper.detailSelect", PostNumber);
		}
